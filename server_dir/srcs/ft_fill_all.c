...
/*   Updated: 2019/01/17 17:10:51 by ldedier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_add_player_data_all(void *content_player, char *data, int *size)
{
	...
	*size += ft_memcpy_ret(&(data[*size]), &(player->score),
			sizeof(player->score));
	*size += ft_memcpy_ret(&(data[*size]), &(player->name_len),
			sizeof(t_name_len));
...
char	*ft_get_buffer_all_cores(t_server *server, t_flag flag, int *size)
{
	...
	if (!(data = (char *)(malloc(total_size))))
		return (NULL);
	*size = 0;
	*size += ft_memcpy_ret(&(data[*size]), &(server->flag),
			sizeof(server->flag));
